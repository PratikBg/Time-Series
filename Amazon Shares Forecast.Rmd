...
```{r}
sample_index<-sample(nrow(amazon.vol.ts),nrow(amazon.vol.ts)*0.8)
amz.train<-amazon.vol.ts[sample_index,]
amz.test<-amazon.vol.ts[-sample_index,]
```
...
Model with AR1
```{r}
arima100 <- arima(amz.train,order=c(1,0,0))
arima100
...
Model with MA1
```{r}
arima001 <- arima(amz.train,order=c(0,0,1))
arima001
...
```{r}
arima101 <- arima(amz.train,order=c(1,0,1))
arima101
...
Model with AR2
```{r}
arima200 <- arima(amz.train,order=c(2,0,0))
arima200
...
```{r}
auto.arima.model <- auto.arima(amz.train)
auto.arima.model
...
```{r}
AR1.predicted <- predict(arima001, newdata = amz.test)
